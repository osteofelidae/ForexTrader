...
lower_band_ask = min(data[:, 1]) + (max(data[:, 1]) - min(data[:, 1]))/margin_div
upper_band_bid = max(data[:, 0]) - (max(data[:, 0]) - min(data[:, 0]))/margin_div

for index1 in range(len(data)-1):
    index = index1 + 1
    point = data[index]

    scoped = d.scope(data=data, index=index, length=100)
    bids = scoped[:, 0]
    asks = scoped[:, 1]
    ...
    buy_signal = ask < lower_band_ask
    sell_signal = bid > upper_band_bid
